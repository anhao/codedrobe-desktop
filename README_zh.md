...
- 应用内一键升级：macOS 由内置更新器原地替换应用，Windows 安装版静默覆盖安装，点「重启并安装」即完成；Portable/MSI 版本回落到下载页面。
...
登录会打开系统浏览器完成 OAuth 2.0 授权码 + PKCE 流程，应用永远接触不到你的密码。申请的权限会在授权确认页逐项列出，之后可随时在网站的「已授权应用」页面撤销。凭证保存在应用数据目录中仅当前用户可读的文件里（权限 0600），与 CodeDrobe CLI 的模型一致；退出登录会先在服务端吊销授权再删除该文件。
